        ...
        $.ajax({
            ...
            success: function(responce){
                ...
                if(data.res == 'OK'){
                    ...
                    form_error.innerHTML = '<span class="red-not">Пользователь не найден...<br>Введите корректный номер телефона</span>';
                }
...
$(document).on('click','.js-send-auth',function(e){
    ...
    if (code_auth != '') {
        $.ajax({
            type: "POST",
            url: "/ajax/sms.php",
            data: JSON.stringify({code_auth: code_auth, action: 'code_auth'}),
            success: function(responce){
                data = JSON.parse(responce)
                console.log(data);
                if(data.res == 'OK'){
        ...
        form_error.innerHTML = '<span class="red-not">Введите код из СМС</span>';
        form_error.style.display = 'block';
        setTimeout(function() {
            form_error.innerHTML = '';
            form_error.style.display = 'none';
