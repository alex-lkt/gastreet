...
<div class="tickets">
	<?foreach($arResult["ITEMS"] as $arElement){
		echo PHP_EOL."<!-- vard ".PHP_EOL;
		print_r($arElement);
		echo PHP_EOL."-->".PHP_EOL;
		?>
		<div class="ticket" style="background-color: #ccc;background-image: url('<?=$arElement["PREVIEW_PICTURE"]["SRC"]?>')" id="<?=$this->GetEditAreaId($arElement['ID']);?>">
			<?
			...
			<div class="ticket__wrapper" >
				<div class="ticket__top">
					<div class="ticket__caption"><?=$arElement["NAME"]?></div>
					<div class="ticket__count">осталось<br> 23 билета</div>
				</div>
				<div class="ticket__desc"><?=$arElement["PREVIEW_TEXT"]?></div>
				
				<?if($arElement["bDisplayButtons"]){?>
				<form action="<?=POST_FORM_ACTION_URI?>" method="post" enctype="multipart/form-data" class="ticket__controls">
				    <div class="quantity">
				        <div class="quantity__body">
				            <a href="#" class="quantity__butt quantity__minus">
				                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
				                    <path d="M19.2 12L4.80005 12" stroke="#1E1E1E" stroke-width="2" stroke-linecap="round"></path>
				                </svg>
				            </a>
				            <input type="number" value="1" class="quantity__input" min="1">
				            <a href="#" class="quantity__butt quantity__plus">
				                <svg xmlns="http://www.w3.org/2000/svg" width="24" height="24" viewBox="0 0 24 24" fill="none">
				                    <path d="M12.0001 4.8L12 19.2M19.2 12L4.80005 12" stroke="#1E1E1E" stroke-width="2" stroke-linecap="round"></path>
				                </svg>
				            </a>
				        </div>
				        <input type="hidden" name="<?echo $arParams["ACTION_VARIABLE"]?>" value="BUY">
						<input type="hidden" name="<?echo $arParams["PRODUCT_ID_VARIABLE"]?>" value="<?echo $arElement["ID"]?>">
						<input type="submit" class="button button-big ticket__button" name="<?echo $arParams["ACTION_VARIABLE"]."ADD2BASKET"?>" value="Добавить билеты">
				    </div>
				</form>
				<?}else{?>
				<div class="ticket__controls">
					<a href="#" class="button button-big ticket__button">Подать заявку</a>
				</div>
				<?}?>
			</div>
		</div>
